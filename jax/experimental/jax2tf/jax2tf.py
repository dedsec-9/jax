...
def convert(fun: Callable,
  ...
  Args:
    ...
    polymorphic_shapes: Specifies input shapes to be treated polymorphically
      ...
      with surrounding parentheses: "(batch, ...)".  The conversion fails if it
        cannot ensure that the it would produce the same sequence of TF ops for
        any non-zero values of the dimension variables.
      See [the
        README](https://github.com/google/jax/blob/master/jax/experimental/jax2tf/README.md#shape-polymorphic-conversion)
        for more details.
  ...
  def converted_fun(*args: TfVal) -> TfVal:
    ...
    if polymorphic_shapes is None:
      ...
      if not isinstance(polymorphic_shapes,
                        Sequence) or len(args) != len(polymorphic_shapes):
        msg = (
            "polymorphic_shapes must be a sequence with the same length as the"
            " argument list "
...
def _interpret_subtrace(main: core.MainTrace,
  ...
  out_tracers: Iterable[TensorFlowTracer] = map(trace.full_raise,
                                                outs)  # type: ignore
...
def _eval_shape(shape: Sequence[shape_poly.DimSize]) -> Sequence[TfVal]:
  ...
  return tuple(_shape_env[d]
               if type(d) is shape_poly.DimVar else d  # type: ignore[index]
               for d in shape)
